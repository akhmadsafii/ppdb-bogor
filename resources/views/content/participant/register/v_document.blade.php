                                    ...
                                    <div class="card-body text-center">
                                        @php
                                            ...
                                            if ($participant['file'] != 'user.png') {
                                                $img = Helper::showImage($participant['file']);
                                            }
                                        @endphp
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('body').on('submit', '#formSubmit', function(e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('participant.register.document.send') }}",
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
                ...
                $(document).on('click', '.delete', function() {
                    ...
                    if (id) {
                        if (confirm("Apa kamu yakin ingin menghapus data ini?") == true) {
                            $.ajax({
                                url: "{{ route('participant.register.document.delete') }}",
                                type: "DELETE",
                                data: {
                                    id
                                },
                ...
                $(document).on('click', '.edit', function() {
                    ...
                    let loader = $(this);
                    $.ajax({
                        url: "{{ route('participant.register.document.detail') }}",
                        data: {
                            id
                        },
                        beforeSend: function() {
                ...
                $(document).on('click', '.status_check', function() {
                    ...
                    $.ajax({
                        url: "{{ route('participant.register.document.update_status') }}",
                        data: {
                            id,
                            value
                        },
